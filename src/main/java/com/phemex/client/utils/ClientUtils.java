...
public final class ClientUtils {
    ...
    public static <T> CompletableFuture<PhemexResponse<T>> sendRequest(ConnectionProp s, HttpOps httpOps, String path, String queryString, String method, String body,
                                                                       TypeReference<PhemexResponse<T>> typeReference) throws PhemexException {
        URI uri = newUriUnchecked(s.getWebUrl(), path);
        Duration expiryDuration = Duration.ofSeconds(s.getExpirySeconds());
        long expiry = Clock.systemUTC().millis() + expiryDuration.toMillis();
        log.debug("expiry is {}, expiry millis {}", expiry, expiryDuration.toMillis());

        return httpOps.sendAsync(
            uri,
            s.getApiKey(), Objects.requireNonNull(s.getApiSecret()).getBytes(StandardCharsets.UTF_8),
            method,
            queryString,
            expiry,
            body,
